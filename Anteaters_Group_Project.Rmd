...
One of the main aims we address in this research is the investigation of factors influencing emotional valence. We fitted a multiple regression model to predict whether the type of emoji and the frequency of use of the emoji, as well as the age of the participants has an effect on their emotional valence. We standardized the emotional valence scores by standard deviation and we mean-centered age for interpretation of coefficients and intercepts at the mean age.



```{r}
#Haven't written the interpretation for this one yet. Also not sure if I'm using (all) the correct predictors. 

m1q2 <- lm(scale(EVS) ~ I(age-mean(age)) + em_cat + freq_emu, data = emoji_clean)
summary(m1q2)
...
# question from Meg - if EVS is already a composite score with 0 as a neutral, doesn't that mean that we don't need to scale it? I think the only think we need to adjust in the model is mean-centering age
```

# Meg working on Q2 and interactions
```{r}
lm(EVS ~ em_cat, data = emoji_clean)

summary(lm(EVS ~ I(age-mean(age)) + em_cat + freq_emu, data = emoji_clean))
...
# see word doc for interpretations 
```
...
Finally, we address the research aim of what factors predicts changes to likelihood in correctly interpreting the Emoji according to the researchers' coding scheme. Given the binary outcome (correct vs. incorrect) and the variables of interested expressed by the researchers, we fitted a multiple logistic regression model to predict correct interpretation with frequency of Emoji use, age, emoji category, and the interaction between the latter two. Like the previous model, we mean-centered age for interpretation of coefficients and intercepts at the mean age. Secondly, we used treatment coding (exact coding scheme in table **Ax**) for the emoji category such that each coefficient is the mean difference between the level and the grand mean, and that all other coefficients are interpreted across levels rather than within a reference level. The final model has the following equation:
