...
First, we investigated if there is a difference in how frequently emojis were used between users of Apple and Android operating systems. Figure \@ref(fig:q1b-violin-plot), shows that the lower quartile and median of the two operating systems were very similar. However,  the upper quartile is higher and the overall range is larger for Apple than Android. It seems that Apple users are more likely to use a higher number of emojis per day. Android users, however, are more densely concentrated around the median. There are only three  unusually high values at 24 emojis per day, all reported by Apple users.
...
Initially, we conducted a two-tailed Welch two sample t-test ($\alpha$ = 0.05) to assess whether the mean frequency of emoji use was different between Android (*Android n* = `r nrow(filter(emoji_clean, opsys == "Android"))` ; *m* = `r round(q1t_test$estimate [1],1)`; *sd* = `r round(sd(filter(emoji_clean, opsys == "Android")$freq_emu), 2)`) and Apple users (*n* = `r nrow(filter(emoji_clean, opsys == "Apple"))`; *m* = `r round(q1t_test$estimate[2],2)`; *sd* = `r round(sd(filter(emoji_clean, opsys == "Apple")$freq_emu), 2)`) to account for the unequal variances in the two groups (see sds; supported by F test of the variance ratio; *F*(`r var_test$parameter[1]`, `r var_test$parameter[2]`) = `r round(var_test$statistic,2)`, *p* `r print_p(var_test$p.value)`). This indicated a significant difference in how frequently emojis were used among Android and Apple users, *t*(`r round(q1t_test$parameter,2)`) = `r round(q1t_test$statistic,2 )`, *p* `r print_p(q1t_test$p.value)`, two-tailed). However, this test assumes normally distributed values of frequency of emoji use in each group, which was not the case as shown by Q-Q plots (see appendix, Figure \@ref(tab:qq-plot-freq)) and corroborated by Shapiro-Wilks tests indicating significantly non-normal distributions (Android: *W* = `r round(shappy_wilks_and$statistic)`, *p* = `r print_p(shappy_wilks_and$p.value)`; Apple: *W* = `r round(shappy_wilks_app$statistic)`,  *p* = `r print_p(shappy_wilks_app$p.value)`). This may affect the estimation of the standard error and, in turn, the reported significance. Given the large sample size (n > 200), we conducted a sensitivity analysis using bootstrapped 95 % confidence intervals, as this way of estimating standard error has no assumptions about the normality of the populations. The bootstrapped estimated mean frequency of emoji use was `r  abs(round(boot_t_test$boot.estimate,2))` lower in Android users compared to Apple users (95 % *CI*[`r round(boot_t_test$boot.conf.int[1], 2)`, `r round(boot_t_test$boot.conf.int[2],2)`]. The non-zero containing confidence interval of this mean difference supports the conclusion from the Welch t-test without the assumption of normal group populations that Apple users indeed use emojis more frequently than Android users.
