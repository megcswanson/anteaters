...
From the violin plots below, we can see that the lower quartile and median of the two operating systems are very similar. However, the upper quartile is higher and overall range is larger for Apple than Android, indicating unequal variance between the operating systems. It seems that Apple users are more likely to use a higher number of emojis per day. Android users however are more densely concentrated around the median. In the data set, we can see there is only one outlier - at 24 emojis per day for one Apple user. We conducted a F-test `r round(var_test$statistic,1)`, and further confirmed unequal variance. Due to unequal variance and sample size between between Android (*n* = `r nrow(filter(emoji_clean, opsys == "Android"))` and Apple users (*n* = `r nrow(filter(emoji_clean, opsys == "Apple"))`, we therefore conducted a Welch two sample t-test to test whether the groups were different in the frequency of emoji-use. 
...
shapiro.test(emoji_clean$freq_emu) # greater than 0.05

wilcox.test(freq_emu ~ opsys, data = filter(emoji_clean, opsys != "Windows"))
...
#Testing normality
c(emoji_clean$age) 
qqline(emoji_clean$age)

q1b_cortest <- cor.test(emoji_clean$age, emoji_clean$freq_emu, method = "spearman")
q1b_cortest
...
A correlation test was conducted to assess whether there is a relationship between the age of an individual, and the frequency by which they use emoji. A total of `r count(emoji_clean)` individuals were included in the analysis, with a mean age of `r round(mean(emoji_clean$age),2 )` (*sd* = `r round(sd(emoji_clean$age),2 )`) and a mean frequency of use of emoji of `r round(mean(emoji_clean$freq_emu),2 )` (*sd* = `r round(sd(emoji_clean$freq_emu),2 )`).There was a strong negative correlation between age of an individual and the frequency of emoji use (*r* = `r round(q1b_cortest$estimate,2)`, *t*(`r round(q1b_cortest$parameter,1)`) = `r round(q1b_cortest$statistic,2)`), *p* `r print_p(q1b_cortest$p.value)`. We therefore reject the null hypothesis that there is no correlation between age and frequency of emoji use. Figure 1 provides a visualization of the relationship. 
...
# The Emotional Effect of Emojis (Renia & MegS)
...
# Accuracy of Emoji Interpretation (JMEP)

*include*

* maximum likelihood estimation

Finally, we address the research aim of what factors make participants more or less likely to correctly interpret emoji according to the researchers' coding scheme. Given the binary outcome (correct vs. incorrect interpretation), we fitted a multiple logistic regression model. We included the same predictors as the previous linear regression; frequency of emoji use first as its effect on interpreting emoji is already known, and additionally age as well as emoji category and the interaction between these to investigate if the effect of age on interpretation differs for the emoji being interpreted. Like the previous model, we mean-centered age, which estimates the coefficients and intercept at the mean age rather than age 0. Secondly, we used treatment coding (exact contrast coding in table **Ax**) for the emoji category since the three different kinds of emoji have no natural reference group. Each coefficient is then the difference between the mean level and the grand mean, and that all other coefficients are interpreted across levels rather than within a reference level. The final model has the following equation (note, E1 and E2 refers to the sum to zero coded variables in Table **Ax**):
...
q3_plotdata1 <- tibble(
  "I(age-mean(age))" = seq(min(emoji_clean$age), max(emoji_clean$age), 1),
  em_cat = rep(unique(emoji_clean$em_cat), 15)
)

q3_plotdata2 <- tibble(
  freq_emu = seq(min(emoji_clean$freq_emu), max(emoji_clean$freq_emu), 1)
)
```

The model fit had residuals of reasonable size (see Figure Ax), suggesting that no assumptions were violated 

**Table 3.1**
\
*Emoji Interpretation: Analysis of Deviance Table*
...
kable(ano_q3, digits = 2, 
      col.names = c("Df", "Deviance", 
