...
* check for influential values (cooks distances + inf matrix) - Meg - i've started writing up cooks distances analysis
* write up analysis - Meg - i've written up the model but please check
* add interaction plot + comment - Meg - interaction plot has been added but I need some help on what (if anything) I need to comment on 

One of the main aims we address in this research is the investigation of factors influencing emotional valence. We fitted a multiple regression model to predict whether the type of emoji and the frequency of use of the emoji, as well as the age of the participants, has an effect on their emotional valence. We mean-centered age for interpretation of coefficients and intercepts. We additionally used sum-to-zero contrasting for this linear regression model, as there is no intuitive reference level between in the emoji categories. The interaction coefficients in this model refer to the movement between the overall grand mean, represented by the intercept coefficient (*b* = -1.80), and the different emoji categories.
...
Cook's distance was also computed to determine which, if any, data points contributed undue influence on this model. The threshold for this determination was given by $4/(N-k-1)$, in which *N* is the number of observations and *k* is the number of predictors. Given this threshold of 0.017, it was determined that 
...
The interactions in the linear model between age and emoji category were plotted. Across all emoji categories, there were no statistically significant interactions between category and age. For the interaction between the 'loudly crying emoji' [`r emoji::emoji("loudly crying face")`] and age, 

```{r anova-linear-regression, echo=FALSE}
...
* write conclusion/interpretation
...
p_EI <- ggplot(emoji_clean, aes(x = EI, fill = em_cat))+
  ...
  theme_light()+
  theme(legend.key.size = unit(c(.3,.2), units ="cm" ),
...
Given the binary outcome (correct vs. incorrect interpretation; see Figure \@ref(fig:q3-desc-plots)F), we fitted a multiple logistic regression model via maximum likelihood estimation. This estimates the change in log-odds for correct interpretation for each predictor (see eq. \@ref(eq:q3-model)). We included the predictors as outlined above. Like the previous model, we mean-centered age so that the log-odds of the intercept are estimated at the mean age of the sample rather than extrapolated to age 0. Secondly, we used zero-to-sum coding (coding scheme in the Appendix, Table \@ref(tab:cat-code-tab-q3)) for the Emoji category since there was no intuitive reference group. The two regression coefficients for Emoji category would be the difference between the mean log-odds of correct interpretation across Emoji categories and the Emoji-specific mean log-odds for the loudly crying face Emoji and the thumbs up Emoji. We specifically these levels due to the seemingly lower age but lower accuracy for correctly interpreting the thumbs up Emoji given the researchers' interest in these categories interaction with age. Thus, we fitted a model where the intercept was the log-odds of correctly interpreting an Emoji averaged across the different categories for a mean-aged participant reporting using 0 Emoji per day. Equation \@ref(eq:q3-model) (note, E1 and E2 refers to the sum-to-zero coding in Table \@ref(tab:cat-code-tab-q3) in the Appendix):

\begin{align}
\log[ \frac {P(Correct)}{P(Incorrect)}] = & \ \hat\beta_0 + \hat\beta_{1}(Frequency) + \hat\beta_{2}(Age) + \hat\beta_{3}E_{1[thumbs \ up]} + \hat\beta_{4}E_{2[loudly\ crying]} \ + \\ 
& \hat\beta_{5}(Age · E_{1[thumbs \ up]}) + \hat\beta_{6}(Age · E_{2[loudly\ crying]}) + \varepsilon_{i} (\#eq:q3-model)
\end{align} 
...
kable(vif_q3, digits = 2, 
      caption = "Generalised Variance Inflation Factors for Interpretation Accuracy Model",
      booktabs = T) |>
  kable_styling(htmltable_class = "lightable-classic")
```
...
infl_q3[c(13, 75, 218),] |> 
  kable(digits = 2, 
        col.names = c("DFBeta intercept", "DFBeta frequency",
                      "DFBeta age", "DFBeta Emoji category[loudly crying]",
                      "DFBetaEmoji category[thumbs up]",
                      "DFBeta Age:Emoji category[loudly crying]",
                      "DFBeta Age:Emoji category[thumbs up]",
                      "DFFit", "Covariance Ratio", "Cook's Distance", "Hat values"),
        caption = "Measures of Potential Influence for Values Identified in the Plotted Cook's Distances",
        booktabs = T) |>
  kable_styling(htmltable_class = "lightable-classic", full_width = T)
```

```{r influ-obs-tab-q3}
emoji_q3_analysis[c(13, 75, 218), -(8:10)]  |> 
  kable(digits = 2, 
        col.names = c("Name", "Age", "Operating system", "Emoji use frequency",
                      "Emoji category", "Emotional valence score",
                      "Emoji Interpretation"),
        caption = "Observed Values of Potential Influential Observations",
        booktabs = T) |>
  kable_styling(htmltable_class = "lightable-classic", full_width = T)
```

```{r sensitivity analysis}
m2q3 <- glm(EI_bi ~ freq_emu + age_mc*em_cat, data = emoji_q3_analysis[-c(75,218, 13),], family = binomial)
