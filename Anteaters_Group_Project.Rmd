...
A correlation test was conducted to assess whether there is a relationship between the age of an individual, and the frequency by which they use emoji. A total of *n* = 235 individuals where included in the analysis, with a mean age of `r round(mean(emoji_clean$age),2 )`(*sd* = `r round(sd(emoji_clean$age),2 )`) and a mean frequency of use of emoji of `r round(mean(emoji_clean$freq_emu),2 )` (*sd* = `r round(sd(emoji_clean$freq_emu),2 )`).
...
```{r chi square}
 ## which one do we think is answering the question?

# OG chi square
q1_chisq <- chisq.test(table(emoji_clean[, "opsys" != "windows"]$opsys, emoji_clean[, "opsys" != "windows"]$em_cat))
...
For each emoji category, we found no significant difference between the observed proportions of Apple and Android users and an hypthesised set of equal proportions (loudly crying face emoji [`r emoji::emoji("loudly crying face")`]: $\chi^2$ (`r q1_chisq_cry$parameter`) = `r round(q1_chisq_cry$statistic, 2)`, *p* `r print_p(q1_chisq_cry$p.value)`; slightly smiling face  emoji [`r emoji::emoji("slightly smiling face")`]: $\chi^2$ (`r q1_chisq_smile$parameter`) = `r round(q1_chisq_smile$statistic, 2)`, *p* `r print_p(q1_chisq_smile$p.value)`; thumbs up emoji [`r emoji::emoji("thumbs up")`]: $\chi^2$ (`r q1_chisq_thumb$parameter`) = `r round(q1_chisq_thumb$statistic, 2)`, *p* `r print_p(q1_chisq_thumb$p.value)`).

**Orignal**

A Pearson's Chi-squared test of independence indicated no significant association between the operating system used and the emoji presented to the participant ($\chi$^2^(4) = 1.55, *p* = .818), suggesting that the emoji category is not dependent on the particpant's operating system. 
...
Based on the results of the $\chi$^2^ test, we still must determine whether or not there is a balance between the operating systems in the number of emojis they were presented. The null hypothesis for this would state that each category of emoji would be presented equally across all operating systems, however a contingency table showing the counts of each emoji indicate otherwise. In order to visualise this, we can display expected and observed values in mosaic plots (fig. 2.3).

```{r}
p1 <- ggplot(data = filter(emoji_clean, opsys != "windows"), aes(x = em_cat, fill = opsys))+
  geom_bar(position = "dodge") +
  labs(legend = "Operating System")
...
# plot
ggplot(data = t) +
  geom_mosaic(aes(x = product(em_cat, opsys), fill = em_cat)) +   
  labs(y="Do you recline?", x="Is it rude to recline?", title = "Mosaic Plot (2 variables)")

plot(t) | plot(as.table(e))
```

As shown above, there is a clear imbalance in the emoji categories shown to participants with Apple versus Android operating systems. 
...
contrasts(emoji_clean$em_cat)
# need to find estimated mean differences

basemodel <- lm(EVS ~ em_cat, data = emoji_clean)

q2m2 <- lm(EVS ~ I(age-mean(age)) + em_cat + freq_emu, data = emoji_clean)

emmeans(basemodel, pairwise ~ em_cat)
emmeans(q2m2, pairwise ~ em_cat)
# this is a base model: no interactions, just to show the effect of each predictor on EVS
```
...
emmeans(m2intq2, pairwise ~ em_cat * I(age-mean(age)))
...
Across emoji categories, the chance of correctly interpreting an emoji was `r as.numeric(coef(m1q3)[1])`. Which emoji was interpreted and emoji use frequency predicted no significant change in this likelihood. Age, on the other hand, reduced the predicted probability of correct interpretation; for each year above the sample mean, **xx** `r as.numeric(coef(m1q3)[3])` % .
...
```
#Group Memebers' Exam Numbers: "B246814, B244840, B239979, , "
