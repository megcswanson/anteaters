...
# QQplot ----
#this should go in the Appendix 
qqnorm(emoji_clean$freq_emu) # make the plot
...
$$
\log\left[ \frac {P(\operatorname{Interpretation} = \operatorname{Correct})}{P(\operatorname{Interpretation} = \operatorname{Incorrect})} \right] = \hat\beta_0 + \hat\beta_{1}(\operatorname{Frequency}) + \hat\beta_{2}(\operatorname{Age_{mean-centered}}) + \hat\beta_{3}·\operatorname{E_{1[thumbs \ up]}} + \hat\beta_{4}·\operatorname{E_{2[loudly\ crying]}} + \hat\beta_{5}(\operatorname{Age_{mean-centered}} \times \operatorname{E_{1[thumbs \ up]}}) + \hat\beta_{6}(\operatorname{Age_{mean-centered}} \times \operatorname{E_{2[loudly\ crying]}}) + \varepsilon_{i}
$$

We conducted the following tests to address the research question of which factors are associated with more or less likelihood of correctly interpreting the tested Emoji. Firstly, an analysis of deviance was conducted using a $\chi^2$ test to test if each predictor significantly reduced model deviance. Secondly, we investigated the associated regression coefficients to elucidate the direction of association. We examined each regression coefficient, its direction, range with 95 % confidence intervals, and significance with *z*-tests ($H_0: \hat\beta_i \neq 0$; $\alpha$ = .05). We also estimated these for slightly smiling face Emoji category ($\hat\beta_0-(\hat\beta_3+\hat\beta_4)$) and its interaction with age ($\hat\beta_0-(\hat\beta_5+\hat\beta_6)$). Before examining the model, we verified that the model fit did not violate assumptions of logistic regression by inspecting the standardised deviance residuals (a priori criterion range: $|standardised \ deviance \ residual| ≤ 3$; see Figure \@ref(fig:q3-stand-dev-res-plot)).
